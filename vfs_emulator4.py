...
class VFSEmulatorGUIStage4:
    def __init__(self, vfs_path=None, script_path=None):
        ...
        # Главное окно
        self.root = tk.Tk()
        self.root.title("VFS Emulator - Stage 4")
        ...
        # Приветствие и debug
        self.append_output("Welcome to VFS Emulator (Stage 4)")
        self.append_output(f"Debug: VFS Path = {self.vfs_path}")
        self.append_output(f"Debug: Script Path = {self.script_path}")
        self.append_output("Available commands: ls, cd, vfs-save, date, who, uniq, exit")
        self.append_output("")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="VFS Emulator GUI Stage 4")
    ...
    emulator = VFSEmulatorGUIStage4(vfs_path=args.vfs_path, script_path=args.script)
    emulator.run()