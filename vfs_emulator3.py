    ...
    "root": {
        ...
        "children": {
            "folder1": {
                "type": "folder",
                "children": {
                    "file2.txt": {"type": "file", "content": "Hello from file2"}
                }
            },
...
class VFSEmulatorGUIStage3:
    def __init__(self, vfs_path=None, script_path=None):
        ...
        # Главное окно
        self.root = tk.Tk()
        self.root.title("VFS Emulator - Stage 3")
        ...
        # Приветствие и debug
        self.append_output("Welcome to VFS Emulator (Stage 3)")
        self.append_output(f"Debug: VFS Path = {self.vfs_path}")
        self.append_output(f"Debug: Script Path = {self.script_path}")
        self.append_output("Available commands: ls, cd, vfs-save, exit")
        self.append_output("")
    ...
    def execute_command(self, args):
        ...
        if cmd == "ls":
            ...
            folder = get_current_folder()
            if args[1] in folder and folder[args[1]]["type"] == "folder":
    ...
    def flatten_vfs(self, path, node):
        ...
        for name, info in node.items():
            ...
            if info["type"] == "file":
                ...
                rows.append([row_path, "file", content])
            else:
                rows.append([row_path, "folder", ""])
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="VFS Emulator GUI Stage 3")
    ...
    emulator = VFSEmulatorGUIStage3(vfs_path=args.vfs_path, script_path=args.script)
    emulator.run()